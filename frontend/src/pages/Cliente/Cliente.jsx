...
const Cliente = () => {
  ...
  const handleCalendarSlotClick = (timeSlot) => {
            ...
            <div className="calendar-grid">
              {Array.from({ length: 18 }, (_, i) => {
                ...
                return (
                  <div 
                    key={timeSlot} 
                    className={`calendar-slot ${isAvailable ? 'available' : 'booked'} ${isSelected ? 'selected' : ''}`}
                    onClick={() => handleCalendarSlotClick(timeSlot)}
                  >
                    <span className="slot-time">{timeSlot}</span>
                    <span className="slot-status">
                      {isSelected ? 'Selecionado' : (isAvailable ? 'Disponível' : 'Ocupado')}
                    </span>
                  </div>
                );
              })}
